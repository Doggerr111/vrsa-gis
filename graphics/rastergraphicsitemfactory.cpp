...
#include "graphics/webrastergraphicsitem.h"
#include "graphics/filerastergraphicsitem.h"
vrsa::graphics::RasterGraphicsItem* vrsa::graphics::RasterGraphicsItemFactory
                                    ::createForWebDataset(raster::WebRasterDataset *dS, bool addToScene)
{
    WebRasterGraphicsItem* item = new WebRasterGraphicsItem(dS);
    if (!addToScene)
        return item;
    emit rasterGraphicsItemCreated(item);
    return item;
}

vrsa::graphics::RasterGraphicsItem *vrsa::graphics::RasterGraphicsItemFactory
                                    ::createForFileDataset(raster::RasterDataset *dS, bool addToScene)
{
    ...
    FileRasterGraphicsItem *item = new FileRasterGraphicsItem(dS);
...
template<typename T>
void vrsa::graphics::RasterGraphicsItemFactory::processRGBChannels(const std::vector<raster::RasterChannel *> channels, QImage &image)
{
...
QImage vrsa::graphics::RasterGraphicsItemFactory::createRGBImage(std::vector<raster::RasterChannel *> channels)
{
    if (channels.size() != 3)
    {
